...
"Inversion of Control" is a computer science pattern which, applied to React
hooks (or fn components), consist of giving complete control on the internal
state to the user of the hook. The hook will use by default its own reducer for
the internal state, and expose it to the user. That gives him the ability to
provide its own reducer, and still use the hook reducer for actions he's not
interested in re-implementing.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=05%3A%20State%20Reducer&em=yoh.legrand%40gmail.com).
