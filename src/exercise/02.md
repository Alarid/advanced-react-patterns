...
With `React.Children.map` and `React.cloneElement`, a parent component can
provide its children (compound components) with a set a internal props that
won't be accessible to the user of the components. They will be merged with the
props passed by the developer. We can check the type of the children to make
sure we pass the internal state related props only to the allowed components.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=02%3A%20Compound%20Components&em=yoh.legrand%40gmail.com).
