...
With nested compound components, it's way easier to use a context to share
internal state, and exposing a hook to get the context inside the root component

`React.Children.map` is still usefull when caring only about direct descendants
while sharing internal state

It's good practice to give a display name to our context, for developers using
the react dev tools.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=03%3A%20Flexible%20Compound%20Components&em=yoh.legrand%40gmail.com).
